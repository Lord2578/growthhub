  ...
  return (
    <Card>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleAdd} className="space-y-3 mt-2">
              ...
              <div className="space-y-1.5">
                <Label>Current saved (optional)</Label>
      ...
      <CardContent className="space-y-4">
        {goals.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4">No goals yet. Add your first savings goal!</p>
        )}
        {goals.map((goal) => {
          ...
          return (
            <div key={goal.id} className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                {days && (
                  <span className={`text-xs ${days === 'Overdue' ? 'text-destructive' : 'text-muted-foreground'}`}>
                    {days}
                  </span>
                )}
              </div>
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>{format(currentConverted)} saved</span>
                <span>{pct}% of {format(targetConverted)}</span>
              </div>
            </div>
          )
        })}
